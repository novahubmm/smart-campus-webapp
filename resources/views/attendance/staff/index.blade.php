<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('attendance.Staff Attendance') }}</h2>
            </div>
        </div>
    </x-slot>

    <div class="py-6 sm:py-10 overflow-x-hidden" x-data="staffAttendancePage(@js([
        'routes' => [
            'daily' => route('staff-attendance.daily'),
            'monthly' => route('staff-attendance.monthly'),
            'summer' => route('staff-attendance.summer'),
            'annual' => route('staff-attendance.annual'),
            'detail' => route('staff-attendance.detail', ['staff' => '___ID___']),
            'store' => route('staff-attendance.store'),
        ],
        'today' => $today,
            ...
            <div x-show="tab === 'daily'" x-cloak class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 flex items-center gap-3 shadow-sm">
                        ...
                        <div>
                            <p class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('attendance.Total Staff') }}</p>
                            <p class="text-2xl font-bold text-gray-900 dark:text-white" x-text="dailyStats.total"></p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('attendance.All Staff') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('attendance.Daily Staff Attendance') }}</h3>
                        <div class="flex flex-wrap items-center gap-3">
                            <div class="relative">
                                <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"></i>
                                <input type="text" x-model="dailySearch" @input.debounce.300ms="filterDailyList" placeholder="{{ __('attendance.Search by name, ID, department...') }}" class="pl-10 pr-4 py-2 rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 text-sm focus:border-amber-500 focus:ring-amber-500 w-64" />
                            </div>
                            <div class="flex items-center gap-2">
                                <input type="date" x-model="dailyDate" @change="loadDaily" class="rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 text-sm focus:border-amber-500 focus:ring-amber-500" />
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300">{{ __('attendance.Staff ID') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700" x-show="paginatedDailyList.length">
                            <template x-for="row in paginatedDailyList" :key="row.id">
                                <tr>
                                    ...
                                    <td class="px-4 py-3">
                                        <input type="time" :value="getRowStartTime(row.id)" @change="updateStartTime(row.id, $event.target.value)" class="w-24 text-sm rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-amber-500 focus:ring-amber-500" />
                                    </td>
                                    <td class="px-4 py-3">
                                        <input type="time" :value="getRowEndTime(row.id)" @change="updateEndTime(row.id, $event.target.value)" class="w-24 text-sm rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-amber-500 focus:ring-amber-500" />
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-end gap-1">
                                            ...
                                            <a :href="detailUrl(row.id, { start: dailyDate, end: dailyDate })" class="w-8 h-8 rounded-md border border-gray-300 dark:border-gray-600 text-gray-500 dark:text-gray-400 flex items-center justify-center hover:border-amber-400 hover:text-amber-500" title="{{ __('attendance.View') }}">
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="month" x-model="monthFilter" @change="loadMonthly" class="rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 text-sm focus:border-amber-500 focus:ring-amber-500" />
                        </div>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700" x-show="paginatedMonthlyList.length">
                                <template x-for="(row, index) in paginatedMonthlyList" :key="row.id">
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ __('attendance.Staff') }}</td>
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="number" x-model="summerYear" @change="loadSummer" class="w-24 rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 text-sm focus:border-amber-500 focus:ring-amber-500" />
                        </div>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700" x-show="paginatedSummerList.length">
                                <template x-for="(row, index) in paginatedSummerList" :key="row.id">
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 border-r border-gray-200 dark:border-gray-700">{{ __('attendance.Staff') }}</td>
                            ...
                            <tbody x-show="!summerList.length">
                                <tr>
                                    <td colspan="20" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('attendance.No summer data.') }}</td>
                                </tr>
                            </tbody>
                        </table>
                        <!-- Summer Pagination -->
            ...
            <div x-show="tab === 'annual'" x-cloak class="space-y-4">
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex items-center gap-2">
                            <label class="text-sm font-semibold text-gray-600 dark:text-gray-400">{{ __('attendance.Year:') }}</label>
                            <input type="number" x-model="annualYear" @change="loadAnnual" class="w-24 rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 text-sm focus:border-amber-500 focus:ring-amber-500" />
                        </div>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody x-show="!annualList.length">
                                <tr>
                                    <td colspan="40" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">{{ __('attendance.No annual data.') }}</td>
                                </tr>
                            </tbody>
                        </table>
                        <!-- Annual Pagination -->
    ...
    <script>
        function staffAttendancePage(config) {
            return {
                ...
                saveAttendance(staffId, data) {
                    ...
                    .then(() => {
                        // Update local data
                        const row = this.dailyList.find(r => r.id === staffId);
                        if (row) {
