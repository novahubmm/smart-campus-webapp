<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('staff-attendance.index') }}" class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600">
                <i class="fas fa-arrow-left"></i>
            </a>
            <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-200">
                <i class="fas fa-user-cog"></i>
            </span>
            <div>
                <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('attendance.Attendance') }}</p>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('attendance.Staff Attendance Details') }}</h2>
            </div>
        </div>
    </x-slot>

    <div class="py-6 sm:py-8" x-data="staffDetailPage(@js([
        'staffId' => $staff->id,
        'routes' => [
            'detail' => route('staff-attendance.detail', $staff->id),
        ],
        'start' => $start,
        'end' => $end,
        'month' => $month,
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div class="space-y-1">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $staff->user?->name ?? '—' }}</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $staff->employee_id }} • {{ $staff->department?->name ?? '—' }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <label class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('attendance.Month') }}</label>
                        <input type="month" x-model="month" @change="onMonthChange" class="rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 focus:border-purple-500 focus:ring-purple-500" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('attendance.Attendance %') }}</p>
                    <p class="text-3xl font-bold" x-text="summary.attendance_pct + '%'" :class="summary.attendance_pct >= 95 ? 'text-green-600' : summary.attendance_pct >= 85 ? 'text-amber-600' : 'text-red-600'"></p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('attendance.Present') }}</p>
                    <p class="text-3xl font-bold text-green-600" x-text="summary.counts.present"></p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('attendance.Late') }}</p>
                    <p class="text-3xl font-bold text-amber-600" x-text="summary.counts.late"></p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('attendance.Absent') }}</p>
                    <p class="text-3xl font-bold text-red-600" x-text="summary.counts.absent"></p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('attendance.Leave') }}</p>
                    <p class="text-3xl font-bold text-blue-600" x-text="summary.counts.excused"></p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs uppercase text-gray-500 dark:text-gray-400">{{ __('attendance.Off / Holiday') }}</p>
                    <p class="text-3xl font-bold text-purple-600" x-text="(summary.counts.off || 0) + (summary.counts.holiday || 0)"></p>
                </div>
            </div>
    ...
    <script>
        function staffDetailPage(config) {
            return {
                ...
                statusClass(status) {
                    const map = {
                        ...
                        'half-day': 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-100',
                        'on-leave': 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-100',
                    };
