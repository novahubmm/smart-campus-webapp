<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('student_profiles.Edit Student Profile') }}</h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-6 sm:py-10" x-data="{
        ...
        form: {
            ...
            email: @js(old('email', $studentProfile->user?->email)),
            phone: @js(old('phone', $studentProfile->user?->phone)),
            nrc: @js(old('nrc', $studentProfile->user?->nrc)),
            is_active: @js((bool) old('is_active', $studentProfile->user?->is_active)),
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
                <form method="POST" action="{{ route('student-profiles.update', $studentProfile) }}" class="p-6 sm:p-8 space-y-6" enctype="multipart/form-data">
                    ...
                    <div x-show="step === 1" x-cloak class="space-y-4">
                        ...
                        <div class="flex items-start gap-6 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg">
                            <div class="flex-shrink-0">
                                <div class="relative">
                                    ...
                                    >
                                    <div class="absolute -bottom-2 -right-2 w-8 h-8 bg-indigo-500 rounded-full flex items-center justify-center shadow-lg">
                                        <i class="fas fa-user text-white text-sm"></i>
                                    </div>
                                </div>
                            </div>
                            <div class="flex-1">
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">{{ __('student_profiles.Student Photo') }}</label>
                                <input 
                                    type="file" 
                                    name="photo" 
                                    accept="image/jpeg,image/png,image/jpg,image/gif"
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:file:bg-indigo-900/30 dark:file:text-indigo-300"
                                    onchange="previewPhoto(this)"
                                >
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ __('student_profiles.Leave blank to keep current photo.') }} JPG, PNG, GIF. Max 2MB</p>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">{{ __('student_profiles.Portal Password') }} <span class="text-gray-400 text-xs">({{ __('student_profiles.leave blank to keep current') }})</span></label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="student_identifier" x-model="form.student_identifier" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-500" placeholder="STD-XXXX">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                ...
                                <select name="gender" x-model="form.gender" class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    <option value="male">{{ __('student_profiles.Male') }}</option>
                                    <option value="female">{{ __('student_profiles.Female') }}</option>
                                    <option value="other">{{ __('student_profiles.Other') }}</option>
                                </select>
                            </div>
                            <div>
                    ...
                    <div x-show="step === 7" x-cloak class="space-y-4">
                        ...
                        <div class="rounded-lg border border-dashed border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 p-4 text-sm text-gray-700 dark:text-gray-300 space-y-2">
                            ...
                            <p>{{ __('student_profiles.You can revisit any step to adjust data. Leave password blank to keep the current password.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-2">
                        ...
                        <div class="flex items-center gap-3 w-full sm:w-auto">
                            ...
                            <button type="submit" x-show="step === total" class="flex-1 sm:flex-none px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-sm">
                                {{ __('student_profiles.Update Student Profile') }}
                            </button>
                        </div>
                    </div>
                </form>
